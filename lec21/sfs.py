...
class fs:
    ...
    # YOUR CODE, 2013011389
        ...
        parent_block = self.data[parent_inode.addr]
        parent_inode.decRefCnt()
        parent_block.delDirEntry(tfile)
    ...
    # YOUR CODE, 2013011389
        ...
        if parent_block.getFreeEntries() <= 0:
            return -1
        # if the newfile was already in parent dir?
        for (name, inode_id) in parent_block.dirList:
            if name == newfile:
                return -1
        ...
        parent_block.addDirEntry(newfile, target_inum)
    # DONE
        return target_inum
    ...
    def createSoftLink(self, target, newfile, parent):
        ...
        parent_inode = self.inodes[parent_inum]
        parent_block = self.data[parent_inode.data]
        if parent_block.getFreeEntries() == 0:
            return -1
        target_inum = self.createFile(parent, newfile, 'f')
    ...
    # YOUR CODE, 2013011389
        ...
        if ftype == 'd':
            addr = self.dataAlloc()
            data_block = self.data[addr]
            data_block.setType(ftype)
            data_block.addDirEntry('.', inum)
            data_block.addDirEntry('..', parent_inum)
            ref = 2
        # now ok to init inode properly
        inode = self.inodes[inum]
        inode.setAll(ftype, addr, ref)
        # inc parent ref count
        parent_inode.incRefCnt()
        # and add to directory of parent
        parent_block.addDirEntry(newfile, inum)
    # DONE
        return inum
    ...
    # YOUR CODE, 2013011389
        ...
        if curSize == 0:
           ...
           if new_addr != -1:
               self.inodes[inum].setAddr(new_addr)
               self.data[new_addr].setType('f')
               self.data[new_addr].addData(data)
        else:
            return -1
        # no data blocks left
        # write file data
