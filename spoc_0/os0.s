...
00000000  0000080e  LL    0x8 (D 8)			//load register a with port
00000004  00001026  LBL   0x10 (D 16)		//load register b with val
00000008  0000009a  BOUT					//output byte by console
root/usr/os/os0.c  8: ivec(void *isr) { asm(LL,8); asm(IVEC); }
0000000c  00000002  LEV   0x0 (D 0)		
00000010  0000080e  LL    0x8 (D 8)			//load register a with isr
00000014  000000a4  ivec 					//call ivec
root/usr/os/os0.c  9: stmr(int val)   { asm(LL,8); asm(TIME); }
00000018  00000002  LEV   0x0 (D 0)
0000001c  0000080e  LL    0x8 (D 8)			//load register a with val
00000020  000000a7  timer 					//call time
root/usr/os/os0.c  10: halt(val)       { asm(LL,8); asm(HALT); }
00000024  00000002  LEV   0x0 (D 0)
00000028  0000080e  LL    0x8 (D 8)			//load register a with val
0000002c  00000000  halt 					//call halt
...
00000034  0000009d  PSHA					//sp = sp - 8, *sp = a
root/usr/os/os0.c  15:   asm(PSHB);			//sp = sp - 8, *sp = b
00000038  000000a0  PSHB
root/usr/os/os0.c  16: 
root/usr/os/os0.c  17:   current++;
0000003c  00000015  LG    0x0 (D 0)
00000040  ffffff57  SUBI  0xffffffff (D -1)	// current = current + 1
00000044  00000045  SG    0x0 (D 0)
root/usr/os/os0.c  18: 
root/usr/os/os0.c  19:   asm(POPB);			//b = *sp, sp = sp + 8
00000048  000000a1  POPB
root/usr/os/os0.c  20:   asm(POPA);			//a = *sp, sp = sp + 8
0000004c  000000a3  POPA
root/usr/os/os0.c  21:   asm(RTI);			//return from interrupt
...
0000005c  00000045  SG    0x0 (D 0)			//global current = 0
root/usr/os/os0.c  27: 
root/usr/os/os0.c  28:   stmr(1000);
00000060  0003e89e  PSHI  0x3e8 (D 1000)	//push parameter 1000
00000064  ffffb405  JSR   0xffffffb4 (TO 0x1c)//call stmr
00000068  00000801  ENT   0x8 (D 8)			//restore stack
root/usr/os/os0.c  29:   ivec(alltraps);
0000006c  ffffc408  LEAG  0xffffffc4 (D -60)//push the pointer to the function alltraps
00000070  0000009d  PSHA
00000074  ffff9805  JSR   0xffffff98 (TO 0x10)//call ivec
